...
class MoveBehind(State):
    ...
    def execute(self, userdata):
        global TAG_POSE, TAGS_FOUND, isToTheLeft, POINT_BEHIND_TRANS, boxToTheLeft
...
class MoveBaseGo(State):
    def __init__(self, distance=0, horizontal=0, yaw=0, frame="base_link", mode=0):
        State.__init__(self, outcomes=["done"])
        self.distance = distance
    ...
    def execute(self, userdata):
        global CURRENT_POSE, START, END_GOAL, isToTheLeft, MIDDLE_GOAL, SIDE_GOAL, END_GOAL
        ...
        if START and not rospy.is_shutdown():
            ...
            if self.mode == 0:
                ...
                self.move_base_client.send_goal_and_wait(SIDE_GOAL)

            elif self.mode == -3:
                self.move_base_client.send_goal_and_wait(BACK_GOAL)
...
if __name__ == "__main__":
    ...
    with sm:
        ...
        StateMachine.add("TurnToSearchPos", Turn(
            90), transitions={"done": "FindBox"})

        StateMachine.add("FindBox", TurnAndFind(),
                         transitions={"find": "MoveCloser"})

        StateMachine.add("MoveCloser", MoveCloser(),
                         transitions={"close_enough": "MoveToSide"})

        StateMachine.add("MoveToSide", MoveBaseGo(0,0,0,"odom",-2),
                         transitions={"done": "MoveToBack"})

        StateMachine.add("MoveToBack",  MoveBaseGo(0,0,0,"odom",-3), transitions={"done": "StopInFront"})

        StateMachine.add("StopInFront", StopInFront() , transitions={"done": "TurnToPushPos"})

        StateMachine.add("TurnToPushPos", Turn(
            180), transitions={"done": "Push"})

        StateMachine.add("Push", Translate(3, 0.2),
